...
export default function EditorArea({ dashboardPage }: Props) {
  const DashboardComponent = dashboardPage ? dashboardComponents[dashboardPage] : null;
  const tabLabel = dashboardPage ? `${dashboardIcons[dashboardPage]} ${dashboardPage}` : null;
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      >
        ...
        >
          {dashboardPage ? (
            <span style={{ fontSize: 14 }}>{dashboardIcons[dashboardPage]}</span>
          ) : (
