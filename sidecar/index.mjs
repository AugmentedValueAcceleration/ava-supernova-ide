...
// ─── Agent S3 Python Subprocess Manager ──────────────────────────────────────

let agentS3Process = null;
let agentS3Ready = false;
const pendingAgentS3 = new Map();
let agentS3RequestId = 0;

/** Resolve the path to the Agent S3 bridge script */
function getAgentS3ScriptPath() {
  const path = require('path');
  return path.join(__dirname, 'agent-s3-bridge.py');
}

/** Spawn the Agent S3 Python subprocess */
function spawnAgentS3() {
  if (agentS3Process) return;
  const { spawn } = require('child_process');
  const scriptPath = getAgentS3ScriptPath();
  const pythonPath = globalThis._sharedState?.computerUseSettings?.pythonPath || 'python';

  agentS3Process = spawn(pythonPath, [scriptPath], {
  ...
  const rl = readline.createInterface({ input: agentS3Process.stdout });
  rl.on('line', (line) => {
    try {
      const event = JSON.parse(line);
      handleAgentS3Event(event);
    } catch { /* ignore non-JSON */ }
  });

  // Log stderr
  agentS3Process.stderr.on('data', (chunk) => {
    const msg = chunk.toString().trim();
    if (msg) emit({ event: 'info', message: `[Agent S3] ${msg.slice(0, 200)}` });
  });

  agentS3Process.on('exit', (code) => {
    emit({ event: 'info', message: `Agent S3 process exited (code ${code})` });
    agentS3Process = null;
    agentS3Ready = false;
    // Reject all pending requests
    for (const [id, pending] of pendingAgentS3) {
      pending.reject(new Error('Agent S3 process exited'));
    }
    pendingAgentS3.clear();
  });
}

/** Send a command to the Agent S3 Python subprocess */
function sendToAgentS3(cmd, data = {}) {
  if (!agentS3Process) throw new Error('Agent S3 not running');
  const msg = JSON.stringify({ cmd, ...data }) + '\n';
  agentS3Process.stdin.write(msg);
}

/** Send a command and wait for a specific event response */
function agentS3Request(cmd, data = {}, timeoutMs = 30000) {
  return new Promise((resolve, reject) => {
    const id = `as3_${++agentS3RequestId}`;
    const timeout = setTimeout(() => {
      pendingAgentS3.delete(id);
      reject(new Error(`Agent S3 request timed out: ${cmd}`));
    }, timeoutMs);

    pendingAgentS3.set(id, {
      cmd,
      resolve: (result) => { clearTimeout(timeout); resolve(result); },
      reject: (err) => { clearTimeout(timeout); reject(err); },
    });

    sendToAgentS3(cmd, { ...data, _requestId: id });
  });
}

/** Handle events from the Agent S3 Python subprocess */
function handleAgentS3Event(event) {
  const eventType = event.event;

  // Forward info/error to the IDE
  ...
  if (eventType === 'install_status') {
    emit({ event: 'agent_s3_status', installed: event.installed, version: event.version });
    return;
  }
  ...
  if (eventType === 'ready') {
    agentS3Ready = true;
    emit({ event: 'info', message: `Agent S3 ready (${event.platform}, screen ${event.screen?.w}x${event.screen?.h})` });
    // Resolve any pending init request
    for (const [id, pending] of pendingAgentS3) {
      if (pending.cmd === 'init') {
        pendingAgentS3.delete(id);
  ...
  // Action proposed — this is the key event for the two-phase protocol
  if (eventType === 'action_proposed') {
    // Forward to the desktop automation handler in the tool execution flow
    for (const [id, pending] of pendingAgentS3) {
      if (pending.cmd === 'predict') {
        pendingAgentS3.delete(id);
        pending.resolve(event);
        break;
      }
  ...
  // Action skipped
  if (eventType === 'action_skipped') {
    for (const [id, pending] of pendingAgentS3) {
      if (pending.cmd === 'skip_action') {
        pendingAgentS3.delete(id);
        pending.resolve(event);
        break;
      }
...
/** Kill the Agent S3 subprocess */
function killAgentS3() {
  if (agentS3Process) {
    try { sendToAgentS3('cancel'); } catch { /* ignore */ }
    setTimeout(() => {
      if (agentS3Process) {
        agentS3Process.kill();
        agentS3Process = null;
...
/** Check if Agent S3 is installed (quick check, no subprocess needed) */
async function checkAgentS3Install() {
  const { spawn } = require('child_process');
  const pythonPath = globalThis._sharedState?.computerUseSettings?.pythonPath || 'python';
  return new Promise((resolve) => {
    const proc = spawn(pythonPath, ['-c', 'import gui_agents; print(gui_agents.__version__ if hasattr(gui_agents, "__version__") else "installed")'], {
      timeout: 10000,
    });
    let output = '';
...
// ─── Safety Enforcement (between predict and execute) ─────────────────────────

const DANGEROUS_PATTERNS = [
  /delete/i, /remove/i, /erase/i, /format/i,
  /send/i, /submit/i, /pay/i, /purchase/i, /buy/i,
  /shutdown/i, /restart/i, /sign out/i, /log out/i,
];

async function validateDesktopAction(actions, settings) {
  const errors = [];

  // Check permission level
  const permission = settings?.permission || 'full';
  if (permission === 'view_only') {
    return { allowed: false, reason: 'Permission level is View Only — no actions allowed' };
  }

  for (const action of actions) {
    // Navigate mode blocks typing
    if (permission === 'navigate' && (action.type === 'type' || action.type === 'key')) {
      errors.push(`Navigate mode: typing blocked (${action.type})`);
      continue;
    }

    // Check dangerous buttons (if blockDangerous enabled)
    if (settings?.blockDangerous && action.type === 'click') {
      try {
        // Ask Tauri for UI elements near the click point
        const elements = await computerUseRequest('list_ui_elements');
        if (elements?.data) {
          for (const el of elements.data) {
            const name = el.name || '';
            if (DANGEROUS_PATTERNS.some(p => p.test(name))) {
              // Check if the click is near this element
              const dist = Math.sqrt(Math.pow(action.x - (el.cx || el.x), 2) + Math.pow(action.y - (el.cy || el.y), 2));
              if (dist < 50) {
                errors.push(`Dangerous button detected: "${name}" near (${action.x}, ${action.y})`);
              }
            }
          }
        }
      } catch { /* UI element check failed, allow action */ }
    }

    // Check allowed apps whitelist
    if (settings?.allowedApps?.length > 0) {
      try {
        const activeWindow = await computerUseRequest('get_active_window');
        const appName = (activeWindow?.data?.app_name || '').toLowerCase();
        const title = (activeWindow?.data?.title || '').toLowerCase();
        const allowed = settings.allowedApps.some(a =>
          appName.includes(a.toLowerCase()) || title.includes(a.toLowerCase())
        );
        if (!allowed) {
          errors.push(`App not whitelisted: ${activeWindow?.data?.app_name || 'unknown'}`);
        }
      } catch { /* window check failed, allow action */ }
    }
  }
...
async function handleInit(data) {
  try {
    ...
    const sharedState = {
      ...
      // Desktop Control — Agent S3 task handler for desktop_control tool
      desktopTaskHandler: async (task, context) => {
        return new Promise((resolve, reject) => {
          // Send desktop_task command to ourselves (handled in the command loop)
          // This is a simplified handler — the full flow is in the 'desktop_task' command handler
          try {
            spawnAgentS3();
            const cuSettings = config.computerUseSettings || {};

            // For the tool, we emit the task and let the command handler run it
            // The tool result is collected from desktop_complete/error events
            const listener = (event) => {
              if (event.event === 'desktop_complete') {
                resolve({ summary: event.message || 'Desktop task completed.' });
              } else if (event.event === 'error' && event.message?.includes('Desktop task')) {
                reject(new Error(event.message));
              }
            };

            // Temporarily intercept emit for this task
            const origEmit = globalThis._emitFn;
            // Use the command handler directly instead
            resolve({ summary: 'Desktop task dispatched. Check progress in chat.' });
          } catch (err) {
            reject(err);
          }
        });
...
rl.on('line', async (line) => {
  ...
  switch (data.cmd) {
    ...
    case 'set_working_hours':
      ...
      break;
    case 'check_agent_s3': {
      const status = await checkAgentS3Install();
      emit({ event: 'agent_s3_status', ...status });
      break;
    }
    case 'desktop_task': {
      // Full desktop automation flow via Agent S3
      const task = data.task;
      const cuSettings = agent?.sharedState?.computerUseSettings || {};
      const maxSteps = cuSettings.maxSteps || 50;

      if (!task) { emitError('No task specified'); break; }

      try {
        // Spawn Agent S3 if not running
        spawnAgentS3();

        // Init if not ready
        if (!agentS3Ready) {
          const initSettings = {
            groundingModel: cuSettings.groundingModel || 'openai',
            groundingApiKey: cuSettings.groundingApiKey || '',
            groundingBaseUrl: cuSettings.groundingBaseUrl || '',
            mainModel: cuSettings.mainModel || 'gpt-4o',
            mainProvider: cuSettings.mainProvider || 'openai',
          };
          sendToAgentS3('init', { settings: initSettings, platform: process.platform === 'win32' ? 'windows' : process.platform === 'darwin' ? 'macos' : 'linux' });
          // Wait for ready (up to 30s)
          await new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('Agent S3 init timed out')), 30000);
            const check = setInterval(() => {
              if (agentS3Ready) { clearInterval(check); clearTimeout(timeout); resolve(); }
            }, 200);
          });
        }
        ...
        for (let step = 1; step <= maxSteps; step++) {
          emit({ event: 'desktop_progress', step, maxSteps, status: 'predicting', message: `Step ${step}/${maxSteps}: Analysing screen...` });

          // Phase 1: Predict
          const prediction = await agentS3Request('predict', {
            task,
            step,
            maxSteps,
          ...
          // Check if done
          if (prediction.event === 'done') {
            emit({ event: 'desktop_progress', step, maxSteps, status: 'done', message: prediction.summary || 'Task complete', screenshot: prediction.screenshot });
            break;
          }

          // Phase 2: Validate
          const actions = prediction.actions || [];
          const validation = await validateDesktopAction(actions, cuSettings);

          if (!validation.allowed) {
            emit({ event: 'desktop_progress', step, maxSteps, status: 'blocked', message: `Blocked: ${validation.reason}`, screenshot: prediction.screenshot_before });
            sendToAgentS3('skip_action', { reason: validation.reason });
            continue;
          }

          // Phase 3: Confirmation (if needed)
          const confirmMode = cuSettings.confirmMode || 'first_per_app';
          let approved = confirmMode === 'never';

          if (!approved) {
            // Emit confirmation request to the IDE
            const confirmId = `da_${step}_${Date.now()}`;
            emit({
              event: 'desktop_action_confirm',
              id: confirmId,
              step,
              actions,
              thought: prediction.thought,
              screenshot_before: prediction.screenshot_before,
            });

            // Wait for user response
            approved = await new Promise((resolve) => {
              pendingConfirmations.set(confirmId, {
                resolve: (result) => resolve(result.approved !== false),
          ...
          if (!approved) {
            emit({ event: 'desktop_progress', step, maxSteps, status: 'denied', message: 'User denied action' });
            sendToAgentS3('skip_action', { reason: 'user_denied' });
            continue;
          }

          // Phase 4: Execute
          emit({ event: 'desktop_progress', step, maxSteps, status: 'executing', message: `Executing: ${actions.map(a => a.type).join(', ')}` });

          const result = await agentS3Request('execute_action', {}, 30000);

          emit({
            event: 'desktop_progress',
            step,
            maxSteps,
            status: 'step_done',
          ...
          // Delay between steps
          const delay = cuSettings.actionDelay || 500;
          if (delay > 0) await new Promise(r => setTimeout(r, delay));
        }

        emit({ event: 'desktop_complete', message: 'Desktop automation task finished' });

      } catch (err) {
        emitError(`Desktop task failed: ${err.message}`);
      } finally {
        // Kill Agent S3 after task (don't keep it running)
        killAgentS3();
      }
      break;
    }
